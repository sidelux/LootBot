...
var timevar = [];
var rankList = [20,50,75,100,150,200,1000];
...
bot.onText(/^\/gruppi/, function(message) {
	...
	bot.getChatMembersCount(-1001069842056).then(function(data) {
		...
		bot.getChatMembersCount(-1001064571576).then(function(data) {
			...
			bot.getChatMembersCount(-1001076850481).then(function(data) {
				...
				bot.getChatMembersCount(-1001078754923).then(function(data) {
					...
					bot.getChatMembersCount(-1001050988033).then(function(data) {
						...
						bot.sendMessage(message.chat.id, 	"<b>Ufficiali</b>\n" +
										...
										"<a href='https://telegram.me/joinchat/C1OmykAvbzFU80rI5EsU6g'>Lootteria</a> (" + c3 + ") - Riservato alle Lotterie\n" +
										...
										"\n<b>Canali</b>\n" +
										"@xxxguida - Una guida valida e fonte di novità\n" +
										"@xxxgossip - Gossip dai vicoli oscuri di Loot\n" +
										"@annuncixxxbot - Per gli annunci delle aste di oggetti!\n" +
										"@SonoPoveroFaccioGuide - Notizie, alberi crafting, e molto altro\n" +
										"@xxxbotquestions - Domande e sondaggi su xxxlandia!\n" +
										"@BlackListLoot - La lista dei giocatori con comportamenti scorretti\n" +
				...
				if (Object.keys(rows).length > 0){
					...
					if (banlist_id.indexOf(account_id) != -1){
						bot.kickChatMember(message.chat.id, rows[0].account_id).then(result => {
							if (result != "False"){
								bot.sendMessage(message.chat.id, "L'utente è bannato dal gioco, l'ho kickato");
							}
						});
						return;
					}
			...
			connection.query('SELECT * FROM player WHERE nickname = "' + n + '"', function(err, rows, fields) {
				...
				if (Object.keys(rows).length == 0){
					bot.kickChatMember(message.chat.id, accountid).then(result => {
						if (result != "False"){
							bot.sendMessage(message.chat.id, "Utente non è iscritto, l'ho kickato");
						}
					});
					return;
				}
			...
			connection.query('SELECT * FROM player WHERE nickname = "' + n + '"', function(err, rows, fields) {
				...
				if (Object.keys(rows).length > 0){
					...
					if ((lev < min) || (lev > max)){
						bot.kickChatMember(message.chat.id, rows[0].account_id).then(result => {
							if (result != "False"){
								bot.sendMessage(message.chat.id, "L'utente non rispetta i requisiti del livello, l'ho kickato");
							}
						});
						return;
					};
	...
	connection.query('SELECT id, account_id, market_ban FROM player WHERE nickname = "' + message.from.username + '"', function(err, rows, fields) {
		...
		connection.query('SELECT * FROM auction_list WHERE creator_id = ' + player_id, function(err, rows, fields) {
			...
			connection.query('SELECT * FROM auction_list WHERE chat_id = -1001069842056', function(err, rows, fields) {
				...
				connection.query('SELECT item.id, item.value FROM inventory, item WHERE inventory.item_id = item.id AND item.name = "' + oggetto + '" AND inventory.player_id = ' + player_id, function(err, rows, fields) {
					if (err) throw err;
					if (Object.keys(rows).length == 0){
...
bot.onText(/^\/pubblicaasta/, function(message) {
	connection.query('SELECT auction_list.id, last_price, creator_id, last_player, item_id, time_end, nickname, market_ban FROM auction_list, player WHERE player.id = auction_list.creator_id AND auction_list.creator_id = (SELECT id FROM player WHERE nickname = "' + message.from.username + '")', function(err, rows, fields) {
		if (err) throw err;
				...
				for (var i = 0; i < len; i++) {
					...
					connection.query('SELECT craftable, id, value FROM item WHERE name = "' + item + '"', function(err, rows, fields) {
						...
						if (Object.keys(rows).length == 0){
							...
							if (this.price > rows[0].value*10){
								console.log("Prezzo troppo alto per " + this.item + ": " + this.price + "/" + rows[0].value*10);
							}
							if (this.price < rows[0].value){
								bot.sendMessage(message.chat.id, "L'oggetto " + this.item + " ha un prezzo troppo basso, è stato impostato al minimo: " + rows[0].value + "§.");
								this.price = rows[0].value;
...
bot.on('callback_query', function (message) {
	if (message.data.indexOf("asta") != -1){	
		...
		connection.query('SELECT account_id, market_ban, money, id FROM player WHERE nickname = "' + message.from.username + '"', function(err, rows, fields) {
			if (err) throw err;
	...
	connection.query('SELECT id, account_id, market_ban FROM player WHERE nickname = "' + message.from.username + '"', function(err, rows, fields) {
		...
		connection.query('SELECT * FROM public_lottery WHERE creator_id = ' + player_id, function(err, rows, fields) {
			...
			connection.query('SELECT * FROM public_lottery WHERE chat_id = -1001069842056', function(err, rows, fields) {
				...
				connection.query('SELECT item.id FROM inventory, item WHERE inventory.item_id = item.id AND item.name = "' + oggetto + '" AND inventory.player_id = ' + player_id, function(err, rows, fields) {
					if (err) throw err;
					if (Object.keys(rows).length == 0){
			...
			connection.query('SELECT player_id FROM public_lottery_players WHERE lottery_id = ' + lottery_id, function(err, rows, fields) {
				...
				connection.query('SELECT * FROM player WHERE id = ' + extracted, function(err, rows, fields) {
					...
					connection.query('SELECT item.name FROM item WHERE id = ' + item_id, function(err, rows, fields) {
						if (err) throw err;
						var itemName = rows[0].name;
						bot.sendMessage(message.chat.id, "Estrazione per " + itemName + "!\n\nIl vincitore è: @" + nickname + "!");
...
bot.onText(/^\/lotteria(?!p) (.+)|^\/lotteria(?!p)/, function(message, match) {
	...
	connection.query('SELECT id, market_ban, money, account_id FROM player WHERE nickname = "' + message.from.username + '"', function(err, rows, fields) {
		if (err) throw err;
		var player_id = rows[0].id;
		var money = rows[0].money;
...
bot.onText(/^\/lotteriap (.+)|^\/lotteriap/, function(message, match) {
	...
	connection.query('SELECT id, market_ban, account_id, money FROM player WHERE nickname = "' + message.from.username + '"', function(err, rows, fields) {
		if (err) throw err;
		var player_id = rows[0].id;
		var money = rows[0].money;
...
bot.onText(/^\/statistiche/, function(message) {
	connection.query('SELECT MAX(id) As tot, SUM(achievement_count) As achievement, SUM(money) As money, SUM(craft_count) As craft, SUM(mission_count) As miss2 FROM player', function(err, rows, fields) {
		if (err) throw err;
		var tot = rows[0].tot;
			...
			connection.query('SELECT MAX(id) As inv FROM inventory', function(err, rows, fields) {
				...
				connection.query('SELECT COUNT(id) As dragon FROM dragon', function(err, rows, fields) {
					...
					connection.query('SELECT COUNT(*) As chest FROM inventory_chest', function(err, rows, fields) {
						...
						connection.query('SELECT MAX(id) As direct FROM market_direct', function(err, rows, fields) {
							if (err) throw err;
							var direct = rows[0].direct;
							connection.query('SELECT MAX(id) As trade FROM market', function(err, rows, fields) {
								if (err) throw err;
								var trade = rows[0].trade;
										...
										connection.query('SELECT SUM(damage) As dmg FROM boss_damage', function(err, rows, fields) {
											...
											connection.query('SELECT MAX(id) As teamn FROM team', function(err, rows, fields) {
												...
												connection.query('SELECT COUNT(*) As u FROM inventory_rarity WHERE name NOT LIKE "Pietra%" AND rarity = "U"', function(err, rows, fields) {
													if (err) throw err;
													var u = rows[0].u;
													connection.query('SELECT COUNT(*) As revenge, MAX(id) As revengen FROM heist_revenge', function(err, rows, fields) {
														if (err) throw err;
														var rev = rows[0].revenge;
														var revn = rows[0].revengen;
														...
														connection.query('SELECT COUNT(*) As active FROM `last_command` WHERE time LIKE "' + today + '%"', function(err, rows, fields) {
															...
															connection.query('SELECT `AUTO_INCREMENT` As lottery FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = "xxx" AND TABLE_NAME = "public_lottery"', function(err, rows, fields) {
																...
																connection.query('SELECT `AUTO_INCREMENT` As shop FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = "xxx" AND TABLE_NAME = "public_shop"', function(err, rows, fields) {
																	...
																	connection.query('SELECT COUNT(*) As holiday FROM holiday', function(err, rows, fields) {
																		if (err) throw err;
...
bot.onText(/^\/oggetti (.+)|^\/oggetti/, function(message, match) {
	...
	var oggetti = [];
	if (oggetto == undefined){
		bot.sendMessage(message.chat.id, "Inserisci il nome parziale dell'oggetto (es. /oggetti Spada Anti) per visualizzare la lista e la quantità");
		return;
	}
	...
	connection.query('SELECT id FROM player WHERE nickname = "' + message.from.username + '"', function(err, rows, fields) {
		...
		for (var i = 0; i < len; i++) {
			connection.query('SELECT item.name, item.rarity, count(item.name) As num FROM `item`, inventory WHERE item.name like "%' + oggetti[i].trim() + '%" AND inventory.item_id = item.id AND inventory.player_id = ' + player_id + ' GROUP BY item.name', function(err, rows, fields) {
				if (err) throw err;
...
bot.onText(/^\/ricerca (.+)|^\/ricerca/, function(message, match) {
	...
	var o = oggetto.trim();
	var e = 0;
	var text = "Utenti che offrono " + o + ":\n";

	connection.query('SELECT id FROM item WHERE name = "' + o + '"', function(err, rows, fields) {
		if (err) throw err;

		if (Object.keys(rows).length == 0){
			bot.sendMessage(message.chat.id, "L'oggetto specificato non esiste, riprova");
			return;
		}

		var itemId = rows[0].id;

		connection.query('SELECT player.nickname FROM public_lottery, player WHERE player.id = public_lottery.creator_id AND item_id = ' + itemId, function(err, rows, fields) {
			if (err) throw err;

			if (Object.keys(rows).length > 0){
				e = 1;
				for (var i = 0, len = Object.keys(rows).length; i < len; i++) {
					text += "> " + rows[i].nickname + " (Lotteria)\n";
				}
			}					

			connection.query('SELECT player.nickname, public_shop.code, public_shop.price FROM public_shop, player, inventory WHERE inventory.player_id = player.id AND inventory.item_id = ' + itemId + ' AND public_shop.public = 1 AND public_shop.quantity > 0 AND player.id = public_shop.player_id AND public_shop.item_id = ' + itemId + ' GROUP BY nickname ORDER BY price ASC', function(err, rows, fields) {
				if (err) throw err;
				if (Object.keys(rows).length > 0){
					e = 1;
					for (var i = 0, len = Object.keys(rows).length; i < len; i++) {
				...
				if (e == 1){
					...
					bot.sendMessage(message.chat.id, "Non ho trovato nessun offerta in corso per l'oggetto specificato");
				}
			});
		});
	});
...
function getInfo(message, player, house_id, from, account_id){
	connection.query('SELECT id FROM player WHERE nickname="' + player + '"', function(err, rows, fields) {
		...
		connection.query('SELECT item.name FROM player, item WHERE player.id = ' + player_id + ' AND player.weapon_id = item.id', function(err, rows, fields) {
			if (err) throw err;
			var weapon = "-";
			if (Object.keys(rows).length > 0){
				weapon = rows[0].name;
												...
												connection.query('SELECT class.name FROM player, class WHERE player.id = ' + player_id + ' AND player.class = class.id', function(err, rows, fields) {
													...
													connection.query('SELECT * FROM player WHERE id = ' + player_id, function(err, rows, fields) {
														...
														bot.sendMessage(message.chat.id, "<b>Giocatore</b>\n" +
																		...
																		"Credito: " + formatNumber(rows[0].money) + "§", html);
													});
												});
											});
										});
